...
class TestEnvio:
    async def test_membro_envia_e_a_mensagem_aparece(self, session: AsyncSession) -> None:
        ...
        await chat_service.envia_mensagem(
            session, ctx, time.id, MensagemCriar(body="Bom dia, time")
        )
    ...
    async def test_espacos_nas_bordas_sao_removidos(self, session: AsyncSession) -> None:
        ...
        mensagem = await chat_service.envia_mensagem(
            session, ctx, time.id, MensagemCriar(body="  com espacos  ")
        )
        assert mensagem.body == "com espacos"


class TestAcesso:
    async def test_quem_nao_e_do_time_nao_le_a_conversa(
    ...
    async def test_admin_alcanca_qualquer_conversa(self, session: AsyncSession) -> None:
        ...
        await chat_service.envia_mensagem(
            session, await contexto_de(session, lead), time.id, MensagemCriar(body="oi")
        )

        mensagens, _ = await chat_service.lista_mensagens(
            session, await contexto_de(session, admin), time.id
        )
...
class TestOrdemEPaginacao:
    async def test_vem_em_ordem_cronologica(self, session: AsyncSession) -> None:
        ...
        for texto in ("primeira", "segunda", "terceira"):
            await chat_service.envia_mensagem(session, ctx, time.id, MensagemCriar(body=texto))
    ...
    async def test_conversa_curta_nao_oferece_cursor(self, session: AsyncSession) -> None:
        time, lead, _ = await cenario(session)
        ctx = await contexto_de(session, lead)
        await chat_service.envia_mensagem(session, ctx, time.id, MensagemCriar(body="oi"))

        _, cursor = await chat_service.lista_mensagens(session, ctx, time.id)
    ...
    async def test_conversa_longa_pagina_para_tras(self, session: AsyncSession) -> None:
        ...
        for i in range(total):
            await chat_service.envia_mensagem(
                session, ctx, time.id, MensagemCriar(body=f"mensagem {i:03d}")
            )

        recentes, cursor = await chat_service.lista_mensagens(session, ctx, time.id)
        ...
        anteriores, _ = await chat_service.lista_mensagens(
            session, ctx, time.id, antes_de=cursor
        )
        assert anteriores
...
class TestExclusao:
    async def test_autor_apaga_a_propria(self, session: AsyncSession) -> None:
        ...
        ctx = await contexto_de(session, membro)
        mensagem = await chat_service.envia_mensagem(
            session, ctx, time.id, MensagemCriar(body="erro de digitacao")
        )

        apagada = await chat_service.exclui_mensagem(session, ctx, time.id, mensagem.id)
    ...
    async def test_apagada_continua_na_conversa(self, session: AsyncSession) -> None:
        ...
        await chat_service.exclui_mensagem(session, ctx, time.id, alvo.id)

        depois, _ = await chat_service.lista_mensagens(session, ctx, time.id)
        assert len(depois) == 3
        assert [m.deleted_at is not None for m in depois] == [False, True, False]

    async def test_membro_comum_nao_apaga_a_dos_outros(
        self, session: AsyncSession
    ) -> None:
        ...
        with pytest.raises(AcessoNegado):
            await chat_service.exclui_mensagem(
                session, await contexto_de(session, membro), time.id, do_lead.id
            )
    ...
    async def test_lead_modera_a_conversa(self, session: AsyncSession) -> None:
        ...
        apagada = await chat_service.exclui_mensagem(
            session, await contexto_de(session, lead), time.id, do_membro.id
        )
        assert apagada.deleted_at is not None
    ...
    async def test_apagar_duas_vezes_nao_muda_a_data(self, session: AsyncSession) -> None:
        ...
        ctx = await contexto_de(session, lead)
        mensagem = await chat_service.envia_mensagem(
            session, ctx, time.id, MensagemCriar(body="oi")
        )

        primeira = await chat_service.exclui_mensagem(session, ctx, time.id, mensagem.id)
        quando = primeira.deleted_at
        segunda = await chat_service.exclui_mensagem(session, ctx, time.id, mensagem.id)

        assert segunda.deleted_at == quando

    async def test_mensagem_de_outro_time_nao_e_alcancavel(
        self, session: AsyncSession
    ) -> None:
        time, lead, _ = await cenario(session)
        ...
        with pytest.raises(RecursoNaoEncontrado):
            await chat_service.exclui_mensagem(session, ctx, outro.id, mensagem.id)
